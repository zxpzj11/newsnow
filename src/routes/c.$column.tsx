...
function SectionComponent() {
  ...
  // 内联SEO数据生成逻辑
  const columnData = columns[column as keyof typeof columns]
  const seoData = columnData
    ? {
        title: `${columnData.zh} - 新闻聚合 | 实时热点新闻阅读平台`,
        description: getColumnDescription(column),
        keywords: getColumnKeywords(column),
        url: `https://newsnow.superme.work/c/${column}`,
      }
    : {
...
function getColumnDescription(columnId: string) {
  const descriptions = {
  ...
  return descriptions[columnId as keyof typeof descriptions] || "汇聚国内外最新资讯，科技、财经、社会等全方位新闻内容，让您第一时间掌握全球动态"
}

function getColumnKeywords(columnId: string) {
  const keywords = {
  ...
  return keywords[columnId as keyof typeof keywords] || "新闻,热点,实时新闻,科技新闻,财经新闻,国际新闻,国内新闻,新闻聚合,资讯"
}
